...
func runXFRM(uiOut *ui.UI, prompter *ui.Prompter, confDir string) error {
	...
	if err := wrapAbort(computeXfrmPaths(cfg)); err != nil {
		return err
	}
	printXfrmConfigSummary(cfg, uiOut)
	if err := wrapAbort(writeXfrmFiles(cfg, uiOut, prompter)); err != nil {
		return err
	}
...
func collectXfrmInputs(cfg *XfrmConfig, uiOut *ui.UI, prompter *ui.Prompter) error {
...
func selectRPKAlgo(cfg *XfrmConfig, uiOut *ui.UI, prompter *ui.Prompter) error {
	options := []ui.Option{
...
func prepareRPK(cfg *XfrmConfig, uiOut *ui.UI, prompter *ui.Prompter) error {
...
func generateAndWriteRPK(cfg *XfrmConfig) error {
	switch cfg.RPKAlgo {
	case RPKAlgoEd25519:
...
func selectKexGroup(cfg *XfrmConfig, uiOut *ui.UI, prompter *ui.Prompter) error {
...
func computeXfrmPaths(cfg *XfrmConfig) error {
...
func writeXfrmFiles(cfg *XfrmConfig, uiOut *ui.UI, prompter *ui.Prompter) error {
	...
	conn := buildXfrmConn(cfg)
	secrets := buildXfrmSecrets(cfg)
	iface := buildXfrmIface(cfg)
...
func buildXfrmConn(cfg *XfrmConfig) string {
	var b strings.Builder
	fmt.Fprintf(&b, "# Generated by tunnel-helper\n")
...
func buildXfrmSecrets(cfg *XfrmConfig) string {
	var b strings.Builder
	if cfg.AuthMethod == AuthRPK {
...
func buildXfrmIface(cfg *XfrmConfig) string {
	var b strings.Builder
...
func printXfrmConfigSummary(cfg *XfrmConfig, uiOut *ui.UI) {
	authLabel := "PSK"
	if cfg.AuthMethod == AuthRPK {
		authLabel = "RPK"
	}
...
func ensureSwanctlConf(cfg *XfrmConfig, uiOut *ui.UI, prompter *ui.Prompter) error {
...
func printXfrmNextSteps(cfg *XfrmConfig, uiOut *ui.UI) {
	uiOut.HR()
